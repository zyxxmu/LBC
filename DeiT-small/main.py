...
def check_model(model):
    ...
    for i in alphas:
        if i.sum() != len(i):
            ...
            break
    if flg:
        print("The model satisfy the N:M sparsity")
        satisfy_flg = 1
    else:
        print("The model doesn't satisfy the N:M sparsity")

def main(args):
    list1 = list(range(1, 4 + 1))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if not satisfy_flg:
            ...
            if args.k < combinations_num - 1:
                card = 1 + math.ceil((combinations_num-1)*((1-(epoch-args.t_i)/(args.t_f-args.t_i)))**4)
